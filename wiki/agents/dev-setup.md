...
<TBD: filled at bootstrap based on Q2. Examples:

- **Nix flake**: `flake.nix` pins toolchain. Activate via `nix develop` or direnv `use flake`.
- **mise**: `.mise.toml` for runtime versions.
- **rustup**: `rust-toolchain.toml` pins channel.
- **Node**: `.nvmrc` or `package.json#engines`.
- **Python**: `.python-version` for pyenv, or `pyproject.toml` + `uv`.
>
